...
def create_user():
    ...
    if email is None or password is None or username is None:
        return failure_response("Missing email, password, or username", 400)

    success, user = users_dao.create_user(username, email, password)
    ...
    # e=body.get('email')
    # password = body.get("password")
    # if uname is None or e is None or password is None:
    #     return failure_response("Please provide a username, email, and password",400)
...
def update_item(user_id, item_id):
    """
    Endpoint to update an item with id item_id.

    Protected with authentification
    """
    ...
    # Confirm user exists and session token isn't expired
...
def login():
    ...
    # Check if email and password credentials exist in the db
    ...
    if user.session_token == "":
        # We have previously logged out, so create a new session token to log in
        user.renew_session()
        db.session.commit()
